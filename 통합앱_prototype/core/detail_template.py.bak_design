...
def build_detail_html(copy: dict, hero_img: str = "", meta: dict = None) -> str:
    """copy: {title,bullets,body,tags,image_brief,...}, hero_img: data URI 또는 URL, meta: 사양 dict"""
    copy = copy or {}
    meta = meta or {}
    title = _e(copy.get("title", "상품 상세페이지"))
...
<header class="hero"><h1>{title}</h1><p class="tag">{tagline}</p>
...
def build_detail_html_13(copy: dict, hero_img: str = "", meta: dict = None, section_images: dict = None) -> str:
...
<header class="hero"><h1>{title}</h1>{('<p class=tag>'+tagline+'</p>') if tagline else ''}
<div class="badges">{badges}</div>{hero_visual}</header>
{blocks}
<section class="pad"><div class="center"><div class="kicker">Information</div><h2 class="sec">상품 정보</h2><div class="rule"></div></div>
<table>{spec_rows}</table></section>
